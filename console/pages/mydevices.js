  ...
  .done (function( data ) { //Get table data
    if (data["error"] == 0) {
      $.each(data["devices"], function( key, device ) {
        $("#devTable tbody").append("<tr id=\"" + device["deveui"] + "\"><td>" + device["deveui"] + "</td><td>" + device["app_id"] + "</td><td>" + device["dev_id"] + "</td><td>" + device["pseudonym"] + "</td><td>" + device["created"] + "</td><td><a href=\"#mydevices\" class=\"delButton\"><i class=\"fa fa-trash\" aria-hidden=\"true\"></i></a></td></tr>");
      });
